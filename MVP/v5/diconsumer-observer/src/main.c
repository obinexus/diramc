...
/* ------------------------------------------------------------------ */
/*  Cryptographic receipt (Knuth multiplicative hash)                  */
/* ------------------------------------------------------------------ */
static unsigned long receipt(void* p, size_t s)
{
...
static void* consumer(void *arg)
{
    ...
    while (ring_pop(&b)) {
        ...
        /* ----- observer logic ------------------------------------- */
        int ok = 1;
        if (b.priority < 0.1f) {                 // forced eviction
        ...
        /* ----- coherence update (Hamiltonian-Eulerian) ------------ */
        float local_c = (float)atomic_load(&events_ok) /
                        (float)atomic_load(&events_total);
